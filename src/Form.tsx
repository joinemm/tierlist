...
import { Tier, Item } from "./TierListNew";
import ImageDrop from '@zentered/solid-image-crop'
...
export const TierForm: VoidComponent<{
  ...
  const [formstore, { Form, Field }] = createForm<TierformData>();

  const handleSubmit: SubmitHandler<TierformData> = (values, event) => {
    console.log(values);
  ...
  return (
    <div class="bg-black rounded-xl p-8 justify-center text-white">
      <Form class="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <button class="bg-green-800 p-1 rounded-m font-bold" type="submit">
          OK
        </button>
        <button
...
export const ItemForm: VoidComponent<{
  ...
  const [name, setName] = createSignal(props.item != null ? props.item.name : "");
  const [img, setImg] = createSignal(props.item != null ? props.item.image_url : "");
    ...
    <div class="bg-black rounded-xl p-8 justify-center text-white">
      <div class="flex flex-col gap-4 max-w-80">
        ...
        <Show when={preview() !== null}>
          ...
          />
          <button class="bg-blue-800 p-1 rounded-m font-bold" onClick={showCroppedImage}>
            CROP
          </button>
        </Show>
        <Show when={img() !== null}>
        ...
        <Show when={preview() === null}>
          <button class="bg-green-800 p-1 rounded-m font-bold" type="submit" onClick={handleSubmit}>
            OK
          </button>
        </Show>
        <button
