...
const TierComponent = (props: { tier: Tier; items: Item[]; state: State; setState: SetStoreFunction<State>; }) => {
    ...
    <>
      ...
      <Modal>
        <TierForm
          tier={props.tier}
          onSubmit={(data: Tier) => {
            props.setState("tiers", (tiers) => tiers.id == props.tier.id,
              produce((tier: Tier) => { tier.name = data.name; tier.color = data.color })
            )
            setModalOpen(false);
          }}
          onDelete={() => props.setState("tiers", props.state.tiers.filter((tier: Tier) => tier.id !== props.tier.id))}
        />
      </Modal>
    </>
  );
...
const ItemComponent = (props: { item: Item; state: State; setState: SetStoreFunction<State> }) => {
    ...
    <>
      ...
      <Modal>
        <ItemForm
          item={props.item}
          onSubmit={(data: Item) => {
            props.setState("items", (items) => items.id == props.item.id,
              produce((item: Item) => { item.name = data.name; item.image_url = data.image_url })
            )
            setModalOpen(false);
          }}
          onDelete={() => props.setState("items", props.state.items.filter((item: Item) => item.id !== props.item.id))}
        />
      </Modal>
    </>
  );
...
const UnsortedContainer = (props: { items: Item[]; state: State; setState: SetStoreFunction<State>; onNewItem: Function }) => {
    ...
    <>
      ...
      >
        <div class="bg-gray-900 flex flex-wrap w-full min-h-20">
          <SortableProvider ids={props.items.map((item) => item.id)}>
            <For each={props.items}>
              {(item) => <ItemComponent item={item} state={props.state} setState={props.setState} />}
            </For>
          </SortableProvider>
          <button class="h-20 w-20" onClick={() => setModalOpen(true)}>
            <FiPlusSquare color="white" size={40} class="m-auto hover:text-green-400" />
          </button>
        </div>
      </div>
      <Modal>
...
export const TierList = () => {
  ...
  const closestEntity: CollisionDetector = (draggable, droppables, context) => {
    ...
    if (draggable.data.type === "tier") {
      return closestCenter(
        draggable,
        droppables.filter((droppable) => droppable.data.type === "tier" && droppable.id !== UNSORTED_ID),
        context,
      );
    } else {
  ...
  return (
    <div class="flex flex-col flex-1 self-stretch max-w-[656px] m-auto">
      <h1 class="text-4xl text-white m-auto my-8">
        Tierlist Maker
      </h1>
      <DragDropProvider
      ...
      >
        ...
        <SortableProvider ids={sortByOrder(state.tiers).map((tier) => tier.id)}>
          ...
          >
            <For each={sortByOrder(state.tiers).filter((tier) => tier.id !== UNSORTED_ID) as Tier[]}>
              {(tier) => (
                <TierComponent
                  tier={tier}
      ...
      <div class="flex justify-center my-8 gap-4">
        <button
          class=" bg-gray-900 py-1 p-2 rounded-md text-xl text-white hover:bg-gray-700"
          onclick={() => setState("items", (_) => true, produce((item: Item) => { item.tier = UNSORTED_ID }))}
        >
          Unsort items
        </button>
        <button
